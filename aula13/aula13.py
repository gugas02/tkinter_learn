...
class Entrada(object):
    ...
    def cria_elementos3(self):
        ...
        # cria um botão para o terceiro app
        self.third_app = Button(self.frame0, text="App Canvas 3 - Fatias", command=self.app3, bg=branco)
        # empacota o btn
        self.third_app.pack()
        # cria um botão para o quarto app
        self.fourth_app = Button(self.frame0, text="App Canvas 4 - Carinha", command=self.app4, bg=branco)
        # empacota o btn
        self.fourth_app.pack()
        # cria um botão para o quarto app
        self.fifth_app = Button(self.frame0, text="App Canvas 5 - Carinha 2", command=self.app5, bg=branco)
        # empacota o btn
        self.fifth_app.pack()
        # cria um botão para o quarto app
    ...
    def cria_elementos7(self):
        ...
        self.canvas = Canvas(self.frame0, bg="blue", width=canvas_l, height=canvas_a)
        # cria a oval
        self.canvas.create_oval((215, 215), (185, 185), fill="yellow", outline="blue",tag="carinha")
        #self.canvas.create_oval((215, 210), (185, 180), fill="black", outline="blue")
        self.canvas.create_arc((211, 210), (189, 180), fill="yellow", outline="black", start=240, extent=60, style="arc",tag="carinha")
        self.canvas.create_oval((189, 190), (199,200), fill="black", outline="yellow",tag="carinha")
        self.canvas.create_oval((201, 190), (211, 200), fill="black", outline="yellow",tag="carinha")
        # cria o botão de start
        self.comeca = Button(self.frame1, text="START", command=self.start, bg=branco)
        # empacota tudo
        self.frame0.pack()
        self.frame1.pack()
        self.canvas.pack()
    ...
    def cria_elementos8(self):
        ...
        self.frame0.pack()
        self.canvas.pack()
        # associa eventos ao canvas, ou seja, quando determinado evento ocorrer (algo como um click do mouse
        # ou apertar uma tecla do teclado)ele faz alguma ação, nesse caso estamos associando o evento de pressionar
        # os direcionais do teclado com o evento de mover a carinha
        self.canvas.bind('<Up>', self.carinhas_up)
        self.canvas.bind('<Down>', self.carinhas_down)
        self.canvas.bind('<Left>', self.carinhas_left)
        self.canvas.bind('<Right>', self.carinhas_right)
        self.canvas.focus_force()

    def cria_elementos9(self):
        self.carrega_imgs()
        #inter.geometry("%ix%i" % (largura, altura))
        self.frame0 = Frame(self.i)
        self.frame0["bg"] = branco
        ...
        self.frame1["bg"] = "blue"
        # cria um canvas
        self.canvas = Canvas(self.frame0, bg="black", width=canvas_l, height=canvas_a)
        # cria o botão de start
        self.comeca = Button(self.frame1, text="START", command=self.start2, bg=branco)
        # empacota tudo
        self.frame0.pack()
        self.frame1.pack()
        self.canvas.pack()
    ...
    def app4(self):
        self.destroi3()
        self.cria_elementos7()

    def start(self):
        self.update()
        inter.after(30,self.start)

    def update(self):
        self.canvas.move("carinha", self.velocidade_x, self.velocidade_y)
        self.x += self.velocidade_x
        self.y += self.velocidade_y
        if self.x > canvas_l - 20 or self.x < 20:
            self.velocidade_x *= -1

        if self.y > canvas_a - 20 or self.y < 20:
            self.velocidade_y *= -1

    def app5(self):
        self.destroi3()
        self.cria_elementos8()

    def carinhas_up(self, event):
        self.canvas.move("carinha", 0, -10)
        self.y -= 10

    def carinhas_down(self, event):
        self.canvas.move("carinha", 0, 10)
        self.y += 10

    def carinhas_left(self, event):
        self.canvas.move("carinha", -10, 0)
        self.y -= 10

    def carinhas_right(self, event):
        self.canvas.move("carinha", 10, 0)
        self.y += 10

    def app6(self):
        self.destroi3()
        self.cria_elementos9()

    def carrega_imgs(self):
        self.mario_dir = []
        self.mario_r = []
        self.mario_l = []
        for i in range(1, 5):
            # gif, pgm, ppm, pbx --> PIL == Pillow (png, jpeg...)
            self.mario_r.append(PhotoImage(file="imgs/mario_%.1i.ppm" % i))
        for i in range(1, 5):
            # gif, pgm, ppm, pbx --> PIL == Pillow (png, jpeg...)
            self.mario_l.append(PhotoImage(file="imgs/mario_l%.1i.ppm" % i))
        self.mario_dir.append(self.mario_r)
        self.mario_dir.append(self.mario_l)
        self.nos = 0
        self.limite = len(self.mario_l) - 1
        self.dir = 0
    def start2(self):
        self.mario = self.canvas.create_image((self.y, 375), image=self.mario_dir[self.dir][self.nos], tag="mariola")
        # associa eventos ao canvas
        self.canvas.bind('<Left>', self.mario_left)
        self.canvas.bind('<Right>', self.mario_right)
        self.canvas.focus_force()
        inter.after(30,self.start2)

    def mario_left(self, event):
        self.dir = 1
        self.mario = self.canvas.create_image((self.y, 375), image=self.mario_dir[self.dir][self.nos], tag="mariola")
        self.canvas.move("mariola", -10, 0)
        self.y -= 10
        if self.nos<self.limite:
            self.nos += 1
        else:
            self.nos = 0

    def mario_right(self, event):
        self.dir = 0
        self.mario = self.canvas.create_image((self.y, 375), image=self.mario_dir[self.dir][self.nos], tag="mariola")
        self.canvas.move("mariola", 10, 0)
        self.y += 10
